...
import { generateStrategicInsights, StrategicInsight, generateProductForecasts, ProductForecast } from '../services/AnalyticsEngine';
...
const Analytics: React.FC = () => {
    ...
    useEffect(() => {
        if (selectedInsight?.actionType === 'LIQUIDATE' && selectedInsight.metadata?.productId) {
            const p = products.find(prod => prod.id === selectedInsight.metadata.productId);
            if (p) setActionInput(Math.floor(p.price * 0.8).toString()); // Default 20% off
        } else {
            setActionInput('');
        }
    ...
    const executeStrategy = async () => {
        ...
        if (actionType === 'TRANSFER') {
            ...
            const p = products.find(prod => prod.id === metadata.productId);
            if (p && actionInput) {
                await updateProduct({ ...p, price: Number(actionInput) });
                addNotification('SUCCESS', `Price Updated: ${p.name} is now ₹${actionInput}`);
            }
        }
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 relative">
            ...
            <div className="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-slate-800 dark:to-slate-800 p-6 rounded-xl border border-indigo-100 dark:border-slate-700">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1">AI-driven actionable insights prioritized by ROI</p>
                    </div>
                    {insights.length > 0 && (
                        <span className="text-xs font-medium px-2 py-1 bg-white dark:bg-slate-700 rounded border border-indigo-100 dark:border-slate-600 text-indigo-600 dark:text-indigo-400">
                            {insights.length} Opportunities Found
                        </span>
                    )}
                </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                    {insights.length === 0 ? (
                        <div className="p-8 text-center text-slate-500 text-sm">
                            No critical insights detected at this moment. Operations are running within healthy parameters.
                        </div>
                    ) : (
                        insights.map(insight => (
                            <div key={insight.id} className="bg-white dark:bg-slate-700 p-4 rounded-lg border border-slate-200 dark:border-slate-600 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 hover:border-indigo-300 transition-colors group">
                                <div className="flex items-start gap-4">
                                    <div className={`p-3 rounded-full shrink-0 ${insight.type === 'Profit Optimization' ? 'bg-green-100 text-green-600' :
                                        insight.type === 'Risk Mitigation' ? 'bg-red-100 text-red-600' :
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                    ...
                    <div className="space-y-4">
                        {[1, 2, 3].map((_, i) => (
                            <div key={i} className="flex gap-3 items-start p-3 rounded-lg bg-orange-50 dark:bg-orange-900/10 border border-orange-100 dark:border-orange-900/30">
                                <AlertTriangle size={16} className="text-orange-600 mt-0.5" />
                                <div>
                                    <p className="text-xs font-bold text-slate-800 dark:text-slate-200">Low Stock Warning</p>
                                    <p className="text-xs text-slate-500 mt-1">Premium Basmati Rice is below threshold (45kg)</p>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            {selectedInsight && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-md p-6 border border-slate-200 dark:border-slate-700 animate-in zoom-in-95 duration-200">
                        ...
                        {selectedInsight.actionType === 'LIQUIDATE' && (
                            <div className="mb-6">
                                <label className="block text-xs font-medium text-slate-500 mb-1">New Clearance Price (₹)</label>
                                <input
                                    type="number"
                                    value={actionInput}
