...
import { Search, MapPin, ArrowRightLeft, Filter, Plus, History, Edit, Box, X, IndianRupee, Skull, Truck, Tag, AlertTriangle, Calendar, CheckCircle, Info, MoreHorizontal, CheckSquare, Camera, Trash2, ChevronDown, Check, ScanLine, Clock, Wand2, Zap, FileText, Bell } from 'lucide-react';
...
export const Inventory: React.FC = () => {
    ...
    const isDeadStock = (product: Product) => {
        ...
        // If no lastSaleDate, assume it's old stock (dead) if it has quantity
        if (!product.lastSaleDate) {
            return getDisplayStock(product, 'all') > 0;
        }
        const daysSinceSale = (now - new Date(product.lastSaleDate).getTime()) / (1000 * 3600 * 24);
        // Using > 90 days as threshold for visual warning, though strict dead stock is often > 365
        return daysSinceSale > 90 && getDisplayStock(product, 'all') > 0;
    };
    ...
    const filteredProducts = useMemo(() => {
        return products.filter(p => {
            if (activeTab === 'low_stock' && !isLowStock(p, selectedLocation)) return false;
            if (activeTab === 'dead_stock' && !isDeadStock(p)) return false;
            if (activeTab === 'expiring') {
                if (!p.expiryDate) return false;
                const diffDays = (new Date(p.expiryDate).getTime() - new Date().getTime()) / (1000 * 3600 * 24);
                if (diffDays > 30 || diffDays < 0) return false; // Fixed logic: Expiring soon is 0-30 days
            }
    ...
    const alertItems = useMemo(() => {
        ...
        const deadStock = products.filter(isDeadStock);
                                ...
                                {alertItems.deadStock.length > 0 && (
                                    <div className="border-t border-slate-100">
                                        <div className="px-4 py-2 bg-amber-50 text-amber-800 text-xs font-bold uppercase tracking-wider flex items-center gap-2 sticky top-0">
                                            <Skull size={12} /> Dead Stock ({alertItems.deadStock.length})
                                        </div>
                                        <div className="divide-y divide-slate-50">
                                        ...
                                        <tr key={p.id} onClick={() => setViewingProduct(p)} className={`hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group cursor-pointer ${deadStock ? 'bg-amber-50/30' : ''} border-b last:border-0 border-slate-100 dark:border-slate-700/50`}>
                                            ...
                                            <td className="px-4 py-4 text-right">
                                                <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    ...
                                                    {deadStock && (
                                                        <button onClick={(e) => { e.stopPropagation(); handleDeadStockOptimize(p); }} className="p-2 text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Optimize Dead Stock">
                                                            <Zap size={16} />
                                                        </button>
                                                    )}
                                        ...
                                        <div className="space-y-4">
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="col-span-2">
                                                    ...
                                                    <select className="w-full border border-slate-300 dark:border-slate-600 rounded-lg p-2.5 outline-none focus:border-indigo-500 bg-white dark:bg-slate-700 text-slate-900 dark:text-white" value={productForm.supplier} onChange={e => setProductForm({ ...productForm, supplier: e.target.value })}>
