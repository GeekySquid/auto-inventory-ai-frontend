...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  React.useEffect(() => {
    if (token) {
      Promise.all([
        ...
        .then(([prods, locs, custs, supps, salesData, transfersData, tiers]) => {
          setProducts(prods);
          setLocations(locs);
          setCustomers(custs);
          setSuppliers(supps);
          ...
          setTaxTiers(tiers);
          addNotification('INFO', 'Data Synced', 'All data loaded from database');
        })
        .catch(err => {
  ...
  return (
    <AppContext.Provider value={{
      ...
      addGoal, updateGoal, deleteGoal, theme, toggleTheme
    }}>
      {children}
    </AppContext.Provider>
  );
